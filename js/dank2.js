...
NB.notify = function (msg, type, delay, spinner, static) {
  ...
  dialog = $(
    '<div class="flash alert-dismissable alert alert-' +
      type +
      ' alert-static">' +
      spinner_html +
      " " +
  ...
  if (NB.Core.data("designRefresh")) {
    ...
    dialog = $(
      '<div class="alert alert-toast alert-dismissible alert-' +
        type +
        " fade show " +
        staticClass +
        '">\
...
(function (global, factory) {
  ...
  var validCCTLD = regexSupplant(
    RegExp(
      "(?:(?:" +
        ...
        "ভাৰত|ভারত|বাংলা|भारोत|भारतम्|भारत|ڀارت|پاکستان|موريتانيا|مليسيا|مصر|قطر|فلسطين|عمان|عراق|سورية|" +
        "سودان|تونس|بھارت|بارت|ایران|امارات|المغرب|السعودية|الجزائر|الاردن|հայ|қаз|укр|срб|рф|мон|мкд|ею|" +
        "бел|бг|ελ|zw|zm|za|yt|ye|ws|wf|vu|vn|vi|vg|ve|vc|va|uz|uy|us|um|uk|ug|ua|tz|tw|tv|tt|tr|tp|to|" +
        "tn|tm|tl|tk|tj|th|tg|tf|td|tc|sz|sy|sx|sv|su|st|ss|sr|so|sn|sm|sl|sk|sj|si|sh|sg|se|sd|sc|sb|sa|" +
        "rw|ru|rs|ro|re|qa|py|pw|pt|ps|pr|pn|pm|pl|pk|ph|pg|pf|pe|pa|om|nz|nu|nr|np|no|nl|ni|ng|nf|ne|nc|" +
        "na|mz|my|mx|mw|mv|mu|mt|ms|mr|mq|mp|mo|mn|mm|ml|mk|mh|mg|mf|me|md|mc|ma|ly|lv|lu|lt|ls|lr|lk|li|" +
        "lc|lb|la|kz|ky|kw|kr|kp|kn|km|ki|kh|kg|ke|jp|jo|jm|je|it|is|ir|iq|io|in|im|il|ie|id|hu|ht|hr|hn|" +
        "hm|hk|gy|gw|gu|gt|gs|gr|gq|gp|gn|gm|gl|gi|gh|gg|gf|ge|gd|gb|ga|fr|fo|fm|fk|fj|fi|eu|et|es|er|eh|" +
        "eg|ee|ec|dz|do|dm|dk|dj|de|cz|cy|cx|cw|cv|cu|cr|co|cn|cm|cl|ck|ci|ch|cg|cf|cd|cc|ca|bz|by|bw|bv|" +
        "bt|bs|br|bq|bo|bn|bm|bl|bj|bi|bh|bg|bf|be|bd|bb|ba|az|ax|aw|au|at|as|ar|aq|ao|an|am|al|ai|ag|af|" +
        "ae|ad|ac" +
        ")(?=[^0-9a-zA-Z@]|$))"
    )
  );
  ...
  var validateUrlUserinfo = regexSupplant(
    ...
    { validateUrlUnreserved: validateUrlUnreserved, validateUrlPctEncoded: validateUrlPctEncoded, validateUrlSubDelims: validateUrlSubDelims },
    "i"
  );
  ...
  var validateUrlAuthority = regexSupplant(
    ...
    "(?:(#{validateUrlUserinfo})@)?" +
      ...
      // $3 port
      "(?::(#{validateUrlPort}))?",
    { validateUrlUserinfo: validateUrlUserinfo, validateUrlHost: validateUrlHost, validateUrlPort: validateUrlPort },
    "i"
  );
  ...
  var validateUrlUnicodeAuthority = regexSupplant(
    ...
    "(?:(#{validateUrlUserinfo})@)?" +
      ...
      // $3 port
      "(?::(#{validateUrlPort}))?",
    { validateUrlUserinfo: validateUrlUserinfo, validateUrlUnicodeHost: validateUrlUnicodeHost, validateUrlPort: validateUrlPort },
    "i"
  );
...
(function ($) {
  ...
  function truncateText(node, max_length) {
    ...
    if (trailing_whitespace)
      // remove initial whitespace if last text
...
jQuery.fn.confirm = function (options) {
  ...
  return this.each(function () {
    ...
    var $cancel = jQuery(options.buttons.wrapper)
      .append(options.buttons.cancel)
      .click(function () {
        // Check if timeout is set.
        if (options.timeout != 0) {
          clearTimeout(timer);
